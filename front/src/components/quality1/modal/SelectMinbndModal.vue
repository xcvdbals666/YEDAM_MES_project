...
<template>
  <Dialog v-model:visible="props.display" :breakpoints="{ '960px': '75vw' }" :style="{ width: '50vw' }" :modal="true">
    <DataTable ref="dt" v-model:selection="selectedProducts" :value="props.minbnd" dataKey="qio_code" v-if="props.minbnd.length > 0">
      <template #header>
      ...
      <Column field="mat_code" header="자재코드" sortable style="min-width: 12rem"></Column>
      <Column field="mat_name" header="이름" sortable style="min-width: 12rem"></Column>
      <Column field="sum" header="수량" sortable style="min-width: 12rem"></Column>
    </DataTable>
