    ...
    <div class="card flex flex-col gap-4 w-full">
      <div class="font-semibold text-xl flex justify-between">
        ...
        <div class="flex flex-row gap-2">
          ...
          <Button v-if="qualityState == 0 && props.callQiOrder" type="button" label="등록" @click="$emit('submitQiResult')" />
          <Button v-else type="button" label="저장" @click="$emit('updateQiResult')" />
          <Button type="button" @click="$emit('callQiResult')" label="검사결과 불러오기" :disabled="props.callQiOrder" severity="info" />
        </div>
      </div>
