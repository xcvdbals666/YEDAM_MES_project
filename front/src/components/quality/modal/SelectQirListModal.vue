...
<template>
  <Dialog v-model:visible="props.display" :breakpoints="{ '960px': '75vw' }" :style="{ width: '50vw' }" :modal="true">
    <DataTable ref="dt" v-model:selection="selectedProducts" :value="props.qiOrderList" dataKey="prdp_code">
      <template #header>
    ...
    <template #footer>
      <Button label="확인" @click="$emit('selectQirList', selectedProducts)" />
