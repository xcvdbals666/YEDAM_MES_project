...
const props = defineProps({
  ...
  allBomInfo: { type: Array, required: true }
});
</script>

<template>
  <Dialog :visible="props.display" :breakpoints="{ '960px': '90vw' }" :style="{ width: '40vw' }" modal @hide="$emit('close')">
    <DataTable ref="dt" v-model:selection="selectedProducts" :value="props.allBomInfo" dataKey="mat_code" selectionMode="single" v-if="props.allBomInfo.length > 0">
      <template #header>
        <div class="flex flex-wrap gap-2 items-center justify-between">
          <h4 class="m-0">제품 불러오기</h4>
        </div>
      </template>

      <Column selectionMode="single" style="width: 3rem" />
      <Column field="mat_code" header="제품코드" headerClass="header-large" />
      <Column field="mat_name" header="제품명" headerClass="header-large" />
      <Column field="unit_name" header="제품유형" headerClass="header-large" />
    </DataTable>
